class TMX : public BackCollector {
	...
	Vector2 last_camera;
	int mapw;
	const int& gettile(const int layer, const int x, const int y){return layers[layer][x+y*mapw];}
	void fovx(const int draw, const int x, const int y, const int Ax, const int radius) {
		if(!radius) return;
		if(gettile(draw,x,y)>=0)
			tileset[gettile(draw,x,y)]->draw(tracecast(Vector2(x,y)));
		if(gettile(1,x,y)!=-1) return;
